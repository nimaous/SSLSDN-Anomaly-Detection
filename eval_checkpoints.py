    ...
    def __init__(self, args, transform, train=True, label=False, type="cifar10"):
        ...
        if type == "cifar10":
            ...
            self.dataset = datasets.LSUN( root=args.data_path, 
                                          classes='test' if not train else 'train',
                                          transform=transform)       
            
        elif type =='tiny_imagenet':
             self.dataset = datasets.ImageFolder(root=args.data_path+'tiny-imagenet-200/test/', 
                                                 transform=transform) 
        elif type =='imagenet30':
             self.dataset = datasets.ImageFolder(root=args.data_path+'ImageNet30/test/', 
                                                 transform=transform)                  
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dist_url", default="tcp://127.0.0.1:23457", type=str, help="""url used to set up
    ...
    dataset_list = ['cifar100', 'svhn', 'imagenet30', 'lsun', 
...
#     dataset_list = ['cifar10',  'places365']
    ...
    if args.load_features:
        features = torch.load(os.path.join(args.load_features, "features.pth"))
        dataset = features['dataset']
        train_features = features['train']
        test_features = features['test']
        test_features_ood = features['ood']
        if utils.get_rank() == 0:
            if args.use_cuda:
                train_features = train_features.cuda()
                test_features = test_features.cuda()
                test_features_ood = test_features_ood.cuda()
    
    chkpt_list = [{'cifar10_augShift': 'checkpoints/checkpoint_NoImgNet.pth'}]
    ood_result_lst = []
    knn_result_lst = []
    with open('auroc_results.txt', 'w+') as file:
        for chkpt in chkpt_list:
            chkpt_name = list(chkpt.keys())[0]
            chkpt_path = list(chkpt.values())[0]
            in_ds = chkpt_name.split('_')[0] 
    ...
    ood_df.to_csv(f'ood_results_df.csv', index=False)
    knn_df.to_csv(f'knn_results_df.csv', index=False)
