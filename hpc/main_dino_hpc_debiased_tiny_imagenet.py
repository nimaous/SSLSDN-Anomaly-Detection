...
def train_dino(args):
    ...
    dataset_aux_path = os.path.join(args.data_path,'300K_random_images.npy')

    dataset_aux = AuxDataImagenet300K(dataset_aux_path,transform=transform_aux)   
...
class DataAugmentation_Contrast(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number, image_size, vit_image_size, aux=False): 
        ...
        self.no_transfo_neg = transforms.Compose([
            transforms.Resize((vit_image_size, vit_image_size), interpolation=Image.BICUBIC),            
            normalize,
            #RandomPixelPerm([3,  vit_image_size, vit_image_size]), 
        ])      
        # first global crop
...
class AuxDataImagenet300K(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        np_img = np.uint8(self.data[item, ...])
        img = Image.fromarray(np_img).convert('RGB')
        if self.transform is not None:
            img = self.transform(img)
        return img , torch.empty(1)
